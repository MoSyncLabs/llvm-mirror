...
MAPIPTargetLowering::MAPIPTargetLowering(MAPIPTargetMachine &tm) :
  ...
  addRegisterClass(MVT::i32, &MAPIP::GR16RegClass);
  addRegisterClass(MVT::i32, &MAPIP::GEXR16RegClass);
...
getRegForInlineAsmConstraint(const std::string &Constraint,
  ...
  if (Constraint.size() == 1) {
    ...
    default: break;
    case 'r':   // GENERAL_REGS
      return std::make_pair(0U, &MAPIP::GR16RegClass);
    }
  }
...
/*
  case CallingConv::MAPIP_INTR: {
    ...
    unsigned VReg =
      RegInfo.createVirtualRegister(&MAPIP::GR16RegClass);
    RegInfo.addLiveIn(MAPIP::A, VReg);
...
// FIXME: varargs
SDValue
MAPIPTargetLowering::LowerCCCArguments(SDValue Chain,
                                        CallingConv::ID CallConv,
                                        bool isVarArg,
                                        const SmallVectorImpl<ISD::InputArg>
                                          &Ins,
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_MAPIP);
...
//  if (CallConv == CallingConv::MAPIP_INTR && !Outs.empty())
//    report_fatal_error("Interrupt handlers cannot return any value");
  ...
  unsigned Opc = (/*CallConv == CallingConv::MAPIP_INTR ?
                  MAPIPISD::RETI_FLAG :*/ MAPIPISD::RET_FLAG);
...
/// TODO: sret.
SDValue
MAPIPTargetLowering::LowerCCCCallTo(SDValue Chain, SDValue Callee,
                                     CallingConv::ID CallConv, bool isVarArg,
                                     bool isTailCall,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == 0)
        StackPtr = DAG.getCopyFromReg(Chain, dl, MAPIP::SP, getPointerTy());
...
SDValue MAPIPTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  SDValue Result = DAG.getTargetGlobalAddress(GV, Op.getDebugLoc(),
                                              getPointerTy(), Offset);
  return DAG.getNode(MAPIPISD::Wrapper, Op.getDebugLoc(),
                     getPointerTy(), Result);
}

SDValue MAPIPTargetLowering::LowerExternalSymbol(SDValue Op,
                                                  SelectionDAG &DAG) const {
...
static MAPIPCC::CondCodes GetSimpleCC(ISD::CondCode CC) {
  switch (CC) {
...
SDValue MAPIPTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
...
SDValue MAPIPTargetLowering::LowerSELECT_CC(SDValue Op,
                                             SelectionDAG &DAG) const {
  ...
  MAPIPCC::CondCodes simpleCC = GetSimpleCC(reverseCC);

  SDVTList VTs = DAG.getVTList(Op.getValueType());
  SmallVector<SDValue, 5> Ops;
  Ops.push_back(DAG.getConstant(simpleCC, MVT::i32));
  Ops.push_back(LHS);
  Ops.push_back(RHS);
  Ops.push_back(TrueV);
  Ops.push_back(FalseV);

  return DAG.getNode(MAPIPISD::SELECT_CC, dl, VTs, &Ops[0], Ops.size());
}

SDValue MAPIPTargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDValue Val = Op.getOperand(0);
  EVT VT      = Op.getValueType();
  DebugLoc dl = Op.getDebugLoc();

  //assert(VT == MVT::i32 && "Only support i16 for now!");
...
SDValue
MAPIPTargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
...
SDValue MAPIPTargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue MAPIPTargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo *MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue MAPIPTargetLowering::LowerROT(SDValue Op,
  ...
  SDValue Ex = DAG.getCopyFromReg(DAG.getEntryNode(), dl, MAPIP::EX, VT,
...
SDValue MAPIPTargetLowering::LowerMUL_LOHI(SDValue Op,
  ...
  SDValue Lo = DAG.getNode(Signed ? MAPIPISD::SMUL : MAPIPISD::UMUL,
                           dl, VTs, Ops, array_lengthof(Ops));
  SDValue Hi = DAG.getCopyFromReg(Lo.getValue(1), dl, MAPIP::EX, VT, Lo.getValue(2));
...
SDValue MAPIPTargetLowering::LowerJumpTable(SDValue Op,
  ...
  return DAG.getNode(MAPIPISD::Wrapper, dl, MVT::i32, TJT);
}
...
const char *MAPIPTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case MAPIPISD::RETI_FLAG:          return "MAPIPISD::RETI_FLAG";
  case MAPIPISD::CALL:               return "MAPIPISD::CALL";
  case MAPIPISD::Wrapper:            return "MAPIPISD::Wrapper";
  case MAPIPISD::BR_CC:              return "MAPIPISD::BR_CC";
  case MAPIPISD::SELECT_CC:          return "MAPIPISD::SELECT_CC";
  case MAPIPISD::SMUL:               return "MAPIPISD::SMUL";
  case MAPIPISD::UMUL:               return "MAPIPISD::UMUL";
  }
}
