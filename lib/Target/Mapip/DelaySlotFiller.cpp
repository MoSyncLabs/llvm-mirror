...
static cl::opt<bool> DisableDelaySlotFiller(
  "disable-mapip-delay-filler",
  cl::init(false),
  cl::desc("Disable the Mapip delay slot filler."),
  cl::Hidden);
...
/// createMapipDelaySlotFillerPass - Returns a pass that fills in delay
/// slots in Mapip MachineFunctions
///
FunctionPass *llvm::createMapipDelaySlotFillerPass(TargetMachine &tm) {
...
bool Filler::runOnMachineBasicBlock(MachineBasicBlock &MBB) {
  ...
  for (MachineBasicBlock::iterator I = MBB.begin(); I != MBB.end(); ++I)
    if (I->hasDelaySlot()) {
      ...
      if (D == MBB.end())
        BuildMI(MBB, ++J, I->getDebugLoc(), TII->get(MAPIP::NOP));
      else
        MBB.splice(++J, &MBB, D);
      ...
      if (needsUnimp(I, structSize)) {
        ...
        BuildMI(MBB, ++J, I->getDebugLoc(),
                TII->get(MAPIP::UNIMP)).addImm(structSize);
      }
    }
  return Changed;
}
                       ...
                       MachineBasicBlock::iterator slot)
{
  ...
  if (slot->getOpcode() == MAPIP::RET)
    return MBB.end();

  if (slot->getOpcode() == MAPIP::RETL) {
    --I;
    if (I->getOpcode() != MAPIP::RESTORErr)
      return MBB.end();
    //change retl to ret
    slot->setDesc(TII->get(MAPIP::RET));
    return I;
  }
                           ...
                           MachineBasicBlock::iterator candidate)
{
  if (candidate == MBB.begin())
    return false;
  if (candidate->getOpcode() == MAPIP::UNIMP)
    return true;
  --candidate;
...
bool Filler::needsUnimp(MachineBasicBlock::iterator I, unsigned &StructSize)
{
  ...
  case MAPIP::CALL: structSizeOpNum = 1; break;
  case MAPIP::JMPLrr:
  case MAPIP::JMPLri: structSizeOpNum = 2; break;
  }
